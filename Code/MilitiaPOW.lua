...
--             print("HUDA_MilitiaPOW:RollForRevolt", sectorId, roll, prisonData.riskPercent)
...
--     for _, pow in ipairs(pows) do
--         pow = gv_UnitData[pow.session_id]
--         if pow.Affiliation == affiliation then            
--             self:ArmPOW(pow)
--             table.insert(freedPows, pow.session_id)
--         else
--             pow:Die()
--         end
--     end
...
--     CombatLog("important",
--         T { 964788160766, "<number> were freed from your prison in sector <sector>", number = #freedPows, sector = sectorId })
-- end

-- function HUDA_MilitiaPOW:Revolt(sectorId)
--     local unitIds = {}

--     for _, unit in ipairs(gv_HUDA_CapturedPows[sectorId]) do
--         unit = gv_UnitData[unit.session_id]
--         self:ArmPOW(unit)
--         table.insert(unitIds, unit.session_id)
--     end
...
--     local ownUnits = #sector.ally_and_militia_squads > 0

--     local revoltResult = ownUnits and T { 964788160766081423, "and are engaging the guards!" } or
--         T { 964788160766081512, "and took over the sector!" }

--     local dlg = CreateMessageBox(popupHost, Untranslated("Prison Break!"),
--     TConcat({ T { 9647881607660815, "The Prisoners in sector <sector> revolted ", sector = sectorId }, revoltResult }, ""))
...
--     print("HUDA_MilitiaPOW:ArmPOW", weaponId)
