...
function HUDA_MilitiaPOW:GetPrisonDescription(sectorId)
	...
	if prisonInfo then
		return T { 1000999105101280817, Untranslated("<newline><em>POW INFORMATION</em><newline><newline>Prisoners: <prisoners>/<capacity><newline>Revolt risk: <risk>"), prisoners = Untranslated(prisonInfo.current), capacity = Untranslated(prisonInfo.max), risk = Untranslated(prisonInfo.risk) }
	end
end
...
function Unit:ShouldGetDowned(hit_descr)
	...
	if not self:IsMerc() then
		...
		CombatLog("debug",
			Untranslated { "Should get downed roll. roll: <downr>, overkill: <overk>, threshold: <thres>", downr = downroll, overk = overkill, thres = threshold })
