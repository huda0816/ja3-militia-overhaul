...
function HUDA_MilitiaPOWInterrogation:HandleEscape(sectorId, goodCop, badCop)
    return {
        ...
        text = T { 08162319880486560821, "During a tense interrogation, <goodcop>'s soft approach and <badcop>'s harsh tactics were unexpectedly thwarted when their clever prisoner escaped, exploiting a brief distraction. This unforeseen breakout sparked an immediate investigation, leaving both mercs to reflect on their underestimated foe.", { goodcop = goodCop.Nick, badcop = badCop.Nick } },
        remove = true
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleDeath(sectorId, goodCop, badCop)
    ...
    return {
        ...
        text = T { 081623198804865608212082, "In the stark interrogation room, <killer>, infamous for his harsh methods, intensely questioned the prisoner. The interrogation quickly spiraled, resulting in the prisoner's unexpected death, which shook loyalty across cities and stained <killer>'s fearsome reputation.", killer = killer.Nick },
        remove = true
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleHurt(sectorId, goodCop, badCop)
    ...
    return {
        headLine = T { 08162319880486560823083, "<interrogator> was hurt", { interrogator = unit.Nick } },
        text = T { 08162319880486560824084, "During an intense interrogation, <interrogator>, acclaimed for his effective techniques, suffered an injury at the hands of the prisoner. This unforeseen turn of events necessitated the prisoner's swift return to confinement and cast a shadow over <interrogator>'s otherwise exemplary performance.", { interrogator = unit.Nick } }
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleNothing(sectorId, goodCop, badCop)
    return {
        ...
        text = T { 08162319880486560821086, "Faced with persistent silence, the <goodcop> and <badcop> could not extract any information from the steadfast prisoner. The session concluded, leaving a lingering hope for more fruitful outcomes in subsequent interrogations.", goodcop = goodCop.Nick, badcop = badCop.Nick }
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleIntel(sectorId, goodCop, badCop)
    ...
    local intel_sectors = HUDA_TableKeys(table.filter(gv_Sectors,
        function(i, o) return o.Intel and not o.intel_discovered end))
    ...
    return {
        ...
        text = T { 08162319880486560821088, "The interrogation proved successful, yielding crucial information about sector <sector> from the prisoner. This intel marks a significant breakthrough in our operations.", sector = randSector },
        remove = true
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleStash(sectorId, goodCop, badCop)
    ...
    return {
        ...
        text = T { 081623198804865608210810, "The prisoner's cooperation led us to his concealed stash, where we discovered <em><got></em>. This valuable find adds a significant boost to our resources.", got = g_Classes[result].DisplayName },
        remove = true
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleRansom(sectorId, goodCop, badCop)
    ...
    return {
        ...
        text = T { 081623198804865608210812,
            "Upon learning of the prisoner's affluent family ties, we initiated contact for a ransom. This strategy proved fruitful, as we received $<money> for facilitating his release, bolstering our funds.", money = money },
        remove = true
    }
end
...
function HUDA_MilitiaPOWInterrogation:HandleMilitia(sectorId, goodCop, badCop)
    ...
    return {
        ...
        text = T(081623198804865608210814, "Through persuasive efforts, we successfully convinced the prisoner to join our ranks and become a part of the militia, strengthening our forces."),
        remove = true
    }
end
...
function HUDA_MilitiaPOWInterrogation:OnComplete(op, sector, mercs)
    ...
    if success then
        ...
        if successMargin > 80 and goodCopRating > 50 then
            randMax = 3
        end
