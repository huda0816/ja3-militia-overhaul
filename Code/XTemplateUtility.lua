DefineClass.HUDA_CustomSettingsUtils = {}

-- create a new table from multiple tables or values in args
function HUDA_CustomSettingsUtils.make_table(...)
	local result = {}
...
--     ...
-- }
function HUDA_CustomSettingsUtils.XTemplate_FindElementsByProp(curr, prop, value, multi, ancestors, indices)
	local results = {}
	...
	if curr[1] then
		local new_ancestors = HUDA_CustomSettingsUtils.make_table({ curr }, ancestors and ancestors or nil)
		for i, v in ipairs(curr) do
			local new_indices = HUDA_CustomSettingsUtils.make_table({ i }, indices and indices or nil)
			local result = HUDA_CustomSettingsUtils.XTemplate_FindElementsByProp(v, prop, value, multi, new_ancestors, new_indices)
